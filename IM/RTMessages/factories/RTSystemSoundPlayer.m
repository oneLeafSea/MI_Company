...
NSString * const kRTSystemSoundTypeAIFF = @"aiff";
NSString * const kRTSystemSoundTypeWAV = @"wav";

@interface RTSystemSoundPlayer()
...
- (void)playSoundWithName:(NSString *)filename
          ...
          completionBlock:(RTSystemSoundPlayerCompletionBlock)completionBlock;
...
- (RTSystemSoundPlayerCompletionBlock)completionBlockForSoundID:(SystemSoundID)soundID;
- (void)addCompletionBlock:(RTSystemSoundPlayerCompletionBlock)block
...
{
    RTSystemSoundPlayer *player = [RTSystemSoundPlayer sharedPlayer];
    
    RTSystemSoundPlayerCompletionBlock block = [player completionBlockForSoundID:soundID];
    if (block) {
        block();
...
@implementation RTSystemSoundPlayer

+ (RTSystemSoundPlayer *)sharedPlayer
{
    static RTSystemSoundPlayer *sharedPlayer;
    
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        sharedPlayer = [[RTSystemSoundPlayer alloc] init];
    });
...
- (BOOL)readSoundPlayerOnFromUserDefaults
{
    NSNumber *setting = [[NSUserDefaults standardUserDefaults] objectForKey:kRTSystemSoundPlayerUserDefaultsKey];
...
- (void)toggleSoundPlayerOn:(BOOL)on
{
    ...
    [userDefaults setObject:[NSNumber numberWithBool:on] forKey:kRTSystemSoundPlayerUserDefaultsKey];
