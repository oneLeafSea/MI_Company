...
static NSString *const WebrtcSRWebSocketAppendToSecKeyString = @"258EAFA5-E914-47DA-95CA-C5AB0DC85B11";

static inline int32_t validate_dispatch_data_partial_string(NSData *data);
static inline dispatch_queue_t log_queue();
static inline void SRFastLog(NSString *format, ...);

@interface NSData (WebrtcSRWebSocket)
...
@interface NSString (WebrtcSRWebSocket)
...
@interface NSURL (WebrtcSRWebSocket)
...
@interface _WebrtcSRRunLoopThread : NSThread
...
@implementation NSData (WebrtcSRWebSocket)
...
@implementation NSString (WebrtcSRWebSocket)
...
NSString *const WebrtcSRWebSocketErrorDomain = @"WebrtcSRWebSocketErrorDomain";

// Returns number of bytes consumed. Returning 0 means you didn't match.
// Sends bytes to callback handler;
typedef size_t (^stream_scanner)(NSData *collected_data);

typedef void (^data_callback)(WebrtcSRWebSocket *webSocket,  NSData *data);
...
@interface WebrtcSRWebSocket ()  <NSStreamDelegate>
...
- (void)_sendFrameWithOpcode:(WebrtcSROpCode)opcode data:(id)data;
...
@property (nonatomic) WebrtcSRReadyState readyState;
...
@implementation WebrtcSRWebSocket {
    ...
    __strong WebrtcSRWebSocket *_selfRetain;
...
- (void)setReadyState:(WebrtcSRReadyState)aReadyState;
{
...
{
    assert(_url);
    NSAssert(_readyState == SR_CONNECTING, @"Cannot call -(void)open on WebrtcSRWebSocket more than once");
...
- (void)_performDelegateBlock:(dispatch_block_t)block;
{
    if (_delegateOperationQueue) {
        ...
        if (_delegateDispatchQueue && self.readyState == SR_OPEN) {
...
- (BOOL)_checkHandshake:(CFHTTPMessageRef)httpMessage;
{
    ...
    NSString *concattedString = [_secKey stringByAppendingString:WebrtcSRWebSocketAppendToSecKeyString];
...
- (void)_HTTPHeadersDidFinish;
{
    ...
    if(![self _checkHandshake:_receivedHTTPHeaders]) {
        [self _failWithError:[NSError errorWithDomain:WebrtcSRWebSocketErrorDomain code:2133 userInfo:[NSDictionary dictionaryWithObject:[NSString stringWithFormat:@"Invalid Sec-WebSocket-Accept response"] forKey:NSLocalizedDescriptionKey]]];
        return;
    }
    ...
    if (negotiatedProtocol) {
        ...
        if ([_requestedProtocols indexOfObject:negotiatedProtocol] == NSNotFound) {
            [self _failWithError:[NSError errorWithDomain:WebrtcSRWebSocketErrorDomain code:2133 userInfo:[NSDictionary dictionaryWithObject:[NSString stringWithFormat:@"Server specified Sec-WebSocket-Protocol that wasn't requested"] forKey:NSLocalizedDescriptionKey]]];
            return;
        }
...
- (void)_readHTTPHeader;
{
    ...
    [self _readUntilHeaderCompleteWithCallback:^(WebrtcSRWebSocket *self,  NSData *data) {
...
- (void)_handleFrameHeader:(frame_header)frame_header curData:(NSData *)curData;
{
    ...
    if (frame_header.payload_length == 0) {
        ...
        [self _addConsumerWithDataLength:frame_header.payload_length callback:^(WebrtcSRWebSocket *self, NSData *newData) {
            if (isControlFrame) {
...
- (void)_readFrameContinue;
{
    ...
    [self _addConsumerWithDataLength:2 callback:^(WebrtcSRWebSocket *self, NSData *data) {
        ...
        if (extra_bytes_needed == 0) {
            ...
            [self _addConsumerWithDataLength:extra_bytes_needed callback:^(WebrtcSRWebSocket *self, NSData *data) {
...
@implementation NSURL (WebrtcWebrtcSRWebSocket)
...
static _WebrtcSRRunLoopThread *networkThread = nil;
static NSRunLoop *networkRunLoop = nil;

@implementation NSRunLoop (WebrtcSRWebSocket)

+ (NSRunLoop *)SR_networkRunLoop {
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        networkThread = [[_WebrtcSRRunLoopThread alloc] init];
...
@implementation _WebrtcSRRunLoopThread {
